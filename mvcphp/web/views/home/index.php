
<?php 
    include WEB_ROOT.'/views/layout/header.php';

?>
    <main>
        <section id="section-banner-home">
            <div class="banner-home-slide">
                <div class="banner-home-item">
                    <img src="<?= DOMAINS.'/public/image/background-home.png'?>" alt="">
                </div>

                <div class="banner-home-item">
                    <img src="<?= DOMAINS.'/public/image/background-home.png'?>" alt="">
                </div>

                <div class="banner-home-item">
                    <img src="<?= DOMAINS.'/public/image/background-home.png'?>" alt="">
                </div>
            </div>
        </section>

        <section id="section-advertisement-home">
            <div class="advertisement-home-inner">
                <div class="container-fluid">
                    <div class="row">
                        <div class="advertisement-home-item col-lg-4 col-md-4 col-sm-4 col-12">
                            <img src="<?= DOMAINS .'/public/image/quangcao1.png'?>" alt="">
                        </div>
            
                        <div class="advertisement-home-item col-lg-4 col-md-4 col-sm-4 col-12">
                            <img src="<?= DOMAINS .'/public/image/quangcao2.png'?>" alt="">
                        </div>
            
                        <div class="advertisement-home-item col-lg-4 col-md-4 col-sm-4 col-12">
                            <img src="<?= DOMAINS .'/public/image/quangcao3.png'?>" alt="">
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <!-- Flash sale -->
        <section id="section-flash-sale">
            <div class="container">
                <div class="section-flash-sale-inner">
                    <div class="flash-sale-title">
                        <p>Flash Sale - Deal chớp nhoáng</p>
                    </div>
                    <div class="flash-sale-link">
                        <a href="">Xem chi tiết</a>
                    </div>
                    <div class="time-flash-sale">
                        <div class="time-flash-item">
                            <div class="time-flash-number">
                                <p>0</p>
                            </div>
                            <div class="time-flash-text">
                                <p>Giờ</p>
                            </div>
                        </div>

                        <div class="time-flash-item time-flash-item-center">
                            <div class="time-flash-number">
                                <p>0</p>
                            </div>
                            <div class="time-flash-text">
                                <p>Phút</p>
                            </div>
                        </div>

                        <div class="time-flash-item">
                            <div class="time-flash-number">
                                <p>0</p>
                            </div>
                            <div class="time-flash-text">
                                <p>Giây</p>
                            </div>
                        </div>
                    </div>

                    <div class="list-product">
                        <div class="row">
                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="ribbon ribbon-top-right"><span>33%</span></div>
                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>

                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>

                                    <div class="progress-box">
                                        <div class="best-sale-progress">
                                            <div class="text-progress">Sản phẩm 30 Đã bán</div>
                                            <div class="sale-progress-val" style="width: 32%"></div>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="ribbon ribbon-top-right"><span>33%</span></div>

                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>

                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>

                                    <div class="progress-box">
                                        <div class="best-sale-progress">
                                            <div class="text-progress">Sản phẩm 30 Đã bán</div>
                                            <div class="sale-progress-val" style="width: 32%"></div>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="<?= DOMAINS .'/public/image/heart.png'?>" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="ribbon ribbon-top-right"><span>33%</span></div>

                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>

                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>

                                    <div class="progress-box">
                                        <div class="best-sale-progress">
                                            <div class="text-progress">Sản phẩm 30 Đã bán</div>
                                            <div class="sale-progress-val" style="width: 32%"></div>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="ribbon ribbon-top-right"><span>33%</span></div>

                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>

                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>

                                    <div class="progress-box">
                                        <div class="best-sale-progress">
                                            <div class="text-progress">Sản phẩm 30 Đã bán</div>
                                            <div class="sale-progress-val" style="width: 32%"></div>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <!-- Xu hướng tìm kiếm -->
        <section id="section-top-search">
            <div class="container">
                <div class="title-section">
                    <h2>Xu hướng tìm kiếm</h2>
                </div>

                <div class="top-search-slide">
                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>

                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>

                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>

                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>

                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>

                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>

                    <div class="top-serach-item">
                        <div class="top-serach-item-inner">
                            <div class="top-search-item-image">
                                <img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt="">
                            </div>
                            <a href="" class="name-top-search">Túi chườm nóng giảm đau bụng</a>
                        </div>
                    </div>
                </div>
            </div>
 

        </section>

        <!-- Bán chạy nhất -->
        <div id="section-best-sale">
            <div class="container">
                <div class="title-section">
                    <h2>Sản phẩm bán chạy</h2>
                </div>

                <div class="best-sale-list">
                    <div class="best-sale-list-inner">
                        <div class="row">
                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>
        
                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>
        
                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>
        
                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-3 col-md-3 col-sm-3 col-12 best-sale-item">
                                <div class="best-sale-item-inner">
                                    <div class="best-sale-img">
                                        <a href=""><img src="<?= DOMAINS .'/public/image/topsearch.png'?>" alt=""></a>
                                    </div>
        
                                    <div class="best-sale-info">
                                        <div class="best-sale-name">
                                            <a href="">Đai Chườm Nóng Massage Giảm Đau Bụng Kinh BUMAS BU01</a>
                                        </div>
        
                                        <div class="best-sale-price">
                                            <p>1.0000.000</p>
                                        </div>
        
                                        <div class="best-sale-discount">
                                            <del>500.000</del><span> (50%)</span>
                                        </div>
                                    </div>
        
                                    <div class="best-sale-rate">
                                        <div class="rate-best-item rate-star">
                                            <img src="<?= DOMAINS .'/public/image/star.png'?>" alt="">
                                            <p>4.8 <span>(34)</span></p>
                                        </div>
        
                                        <div class="rate-best-item rate-sold">
                                            <p>2.6k Đã bán</p>
                                            <img src="/public/image/heart.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Chuyên mục -->
        <section id="section-home-category">
            <div class="container-fluid">
                <div class="row">
                    <div class="home-category-big col-12 ">
                        <div class="category-home-img">
                            <a href=""><img src="<?= DOMAINS .'/public/image/backgroundbig.png'?>" alt=""></a>
                        </div>
                    </div>

                    <div class="home-category-small col-lg-6 col-md-6 col-sm-6 col-12">
                        <div class="category-home-img">
                            <a href=""><img src="<?= DOMAINS .'/public/image/backgroundsmaillhome.png'?>" alt=""></a>
                        </div>
                    </div>

                    <div class="home-category-small col-lg-6 col-md-6 col-sm-6 col-12">
                        <div class="category-home-img">
                            <a href=""><img src="<?= DOMAINS .'/public/image/backgroundsmaillhome.png'?>" alt=""></a>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <!-- Banner chinh sách công ty -->
        <section id="section-banner-policy">
            <div class="container-fluid">
                <div class="banner-policy">
                    <img src="<?= DOMAINS.'/public/image/background-home.png'?>" alt="">
                </div>
            </div>
        </section>

        <!-- Bình luận -->
        <section id="section-comment-customer">
            <div class="container">
                <div class="title-section">
                    <h2>Trải nghiệm, đánh giá từ khách hàng </h2>
                </div>

                <div class="title-section-sub">
                    <p>Những đánh giá và cảm nhận từ khách hàng sau khi sử dụng sản phẩm của Bumas</p>
                </div>
                <div class="container">
                    <div class="comment-customer-slide slick">
                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>

                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>

                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>

                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>

                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>

                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>

                        <div class="comment-customer-item" >
                            <div class="comment-customer-img" >
                                <img src="<?= DOMAINS.'/public/image/comment.png'?>" alt="">
                            </div>
                        </div>
                    </div>

                    <div class="comment-text-slide">
                        <div class="comment-text-item">
                            <div class="comment-text-description">
                                Máy massage cổ SKG G7 pro vừa xung điện, vừa chườm nóng, vừa hồng ngoại, vừa mát xa vật lý, với 9 đầu điện cực như 9 ngón tay massage bấm cổ, mô phỏng phương pháp massage Shiatsu của Nhật Bản, với cực kì nhiều chế độ khác nhau: nóng ít, nóng nhiều, cường độ mạnh, nhẹ tạo cảm giác vô cùng dễ chịu, thoải mái, thích thú khi sử dụng.</p>
                            </div>

                            <div class="comment-text-name">
                                Thanh Ngọc
                            </div>

                            <div class="comment-text-position">
                                KOC
                            </div>
                        </div>

                        <div class="comment-text-item">
                            <div class="comment-text-description">
                                Máy1 massage cổ SKG G7 pro vừa xung điện, vừa chườm nóng, vừa hồng ngoại, vừa mát xa vật lý, với 9 đầu điện cực như 9 ngón tay massage bấm cổ, mô phỏng phương pháp massage Shiatsu của Nhật Bản, với cực kì nhiều chế độ khác nhau: nóng ít, nóng nhiều, cường độ mạnh, nhẹ tạo cảm giác vô cùng dễ chịu, thoải mái, thích thú khi sử dụng.</p>
                            </div>

                            <div class="comment-text-name">
                                Thanh Ngọc
                            </div>

                            <div class="comment-text-position">
                                KOC
                            </div>
                        </div>

                        <div class="comment-text-item">
                            <div class="comment-text-description">
                                M1áy massage cổ SKG G7 pro vừa xung điện, vừa chườm nóng, vừa hồng ngoại, vừa mát xa vật lý, với 9 đầu điện cực như 9 ngón tay massage bấm cổ, mô phỏng phương pháp massage Shiatsu của Nhật Bản, với cực kì nhiều chế độ khác nhau: nóng ít, nóng nhiều, cường độ mạnh, nhẹ tạo cảm giác vô cùng dễ chịu, thoải mái, thích thú khi sử dụng.</p>
                            </div>

                            <div class="comment-text-name">
                                Thanh Ngọc
                            </div>

                            <div class="comment-text-position">
                                KOC
                            </div>
                        </div>

                        <div class="comment-text-item">
                            <div class="comment-text-description">
                                Máy massage cổ SKG G7 pro vừa xung điện, vừa chườm nóng, vừa hồng ngoại, vừa mát xa vật lý, với 9 đầu điện cực như 9 ngón tay massage bấm cổ, mô phỏng phương pháp massage Shiatsu của Nhật Bản, với cực kì nhiều chế độ khác nhau: nóng ít, nóng nhiều, cường độ mạnh, nhẹ tạo cảm giác vô cùng dễ chịu, thoải mái, thích thú khi sử dụng.</p>
                            </div>

                            <div class="comment-text-name">
                                Thanh Ngọc
                            </div>

                            <div class="comment-text-position">
                                KOC
                            </div>
                        </div>

                        <div class="comment-text-item">
                            <div class="comment-text-description">
                                Máy1 massage cổ SKG G7 pro vừa xung điện, vừa chườm nóng, vừa hồng ngoại, vừa mát xa vật lý, với 9 đầu điện cực như 9 ngón tay massage bấm cổ, mô phỏng phương pháp massage Shiatsu của Nhật Bản, với cực kì nhiều chế độ khác nhau: nóng ít, nóng nhiều, cường độ mạnh, nhẹ tạo cảm giác vô cùng dễ chịu, thoải mái, thích thú khi sử dụng.</p>
                            </div>

                            <div class="comment-text-name">
                                Thanh Ngọc
                            </div>

                            <div class="comment-text-position">
                                KOC
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </section>

        <!-- Báo chí nói gì về bumas -->
        <section id="section-tell">
            <div class="container-fluid">
                <div class="row">
                    <div class="title-section">
                        <h2>Báo chí nói gì về Bumas</h2>
                    </div>

                    <div class="news-tell">
                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/dantri.png'?>" alt="">
                        </div>

                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/vietnamnet.png'?>" alt="">
                        </div>

                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/dantri.png'?>" alt="">
                        </div>

                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/vietnamnet.png'?>" alt="">
                        </div>

                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/vietnamnet.png'?>" alt="">
                        </div>

                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/dantri.png'?>" alt="">
                        </div>

                        <div class="news-tell-item">
                            <img src="<?= DOMAINS .'/public/image/vietnamnet.png'?>" alt="">
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </main>

    <?php 
    include WEB_ROOT.'/views/layout/footer.php';

?>